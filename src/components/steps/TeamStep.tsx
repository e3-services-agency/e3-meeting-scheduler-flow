            ...
            <div key={member.id} className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 bg-e3-space-blue/70 rounded-lg border border-e3-white/10">
              <div className="flex items-center gap-3 flex-1">
                ...
                <div className="flex-1">
                  ...
                  {/* The property is now role_name, not role */}
                  <p className="text-sm text-e3-white/70">{member.role_name}</p>

                  {/* --- FIX APPLIED HERE: The block below has been removed --- */}
                  {/* The `member.clientTeams` property no longer exists, so this code would crash the page. */}
                  {/* It is safe to remove as we are already on a specific client's page. */}
