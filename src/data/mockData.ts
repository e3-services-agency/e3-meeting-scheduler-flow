...
function generateAvailability(memberId: string): Record<string, string[]> {
  ...
  for (let day = 0; day < 3; day++) {
    ...
    for (let hour = 9; hour < 17; hour++) {
      ...
      const memberIdNum = parseInt(memberId) || 1;
      if (Math.random() > (0.2 + (memberIdNum * 0.1))) {
        const slot = new Date(currentDate);
        slot.setHours(hour, 0, 0, 0);
        availability[dateString].push(slot.toISOString());
      }
      if (Math.random() > (0.3 + (memberIdNum * 0.1))) {
        const slot = new Date(currentDate);
